# Master script to run all tests and generate comparison report (Windows PowerShell)

$ErrorActionPreference = "Stop"

Write-Host "==========================================" -ForegroundColor Cyan
Write-Host "Data Visualization Enhancement Evaluation" -ForegroundColor Cyan
Write-Host "==========================================" -ForegroundColor Cyan
Write-Host ""
...
# Run Project A tests
Write-Host "Step 1: Running Project A (Pre-Enhancement) tests..." -ForegroundColor Yellow
Write-Host "---------------------------------------------------" -ForegroundColor Yellow
Set-Location Project_A_PreFeature_UI
if (Test-Path "run_tests.ps1") {
    & .\run_tests.ps1
} else {
    Write-Host "Error: run_tests.ps1 not found in Project_A_PreFeature_UI" -ForegroundColor Red
    exit 1
}
Set-Location ..

Write-Host ""
Write-Host "Step 2: Running Project B (Post-Enhancement) tests..." -ForegroundColor Yellow
Write-Host "---------------------------------------------------" -ForegroundColor Yellow
Set-Location Project_B_PostFeature_UI
if (Test-Path "run_tests.ps1") {
    & .\run_tests.ps1
} else {
    Write-Host "Error: run_tests.ps1 not found in Project_B_PostFeature_UI" -ForegroundColor Red
    exit 1
}
Set-Location ..
...
Write-Host "  - Project A results: Project_A_PreFeature_UI\results\results_pre.json"
Write-Host "  - Project B results: Project_B_PostFeature_UI\results\results_post.json"
